...
export const useUpdateFullTaskName = (selectedId: string) => {
    const updateFullTaskNameMutation = useMutation({
        mutationKey: [fullTaskApi.baseKey, "name"],
        mutationFn: fullTaskApi.updateFullTaskName,
        onMutate: async (params) => {
            ...
            queryClient.setQueryData(
                ...
                () => {
                    const newData = previousNotes.data?.map(task => (task as FullTaskResponce).full_task_id === params.fullTaskId ? { ...task, name: params.name } : task);
    ...
    return {
        updateFullTaskName: updateFullTaskNameMutation.mutate
    }
}