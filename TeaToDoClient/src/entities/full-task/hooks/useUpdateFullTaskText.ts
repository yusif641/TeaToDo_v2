...
export const useUpdateFullTaskText = (selectedId: string) => {
    const updateFullTaskTextMutation = useMutation({
        mutationKey: [fullTaskApi.baseKey, "text"],
        mutationFn: fullTaskApi.updateFullTaskText,
        onMutate: async (params) => {
            ...
            queryClient.setQueryData(
                ...
                () => {
                    const newData = previousNotes.data?.map(task => (task as FullTaskResponce).full_task_id === params.fullTaskId ? { ...task, text: params.text } : task);
    ...
    return {
        updateFullTaskText: updateFullTaskTextMutation.mutate
    }
}