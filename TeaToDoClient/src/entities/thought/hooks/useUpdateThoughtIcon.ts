...
export const useUpdateThoughtIcon = (selectedId: string) => {
    const updateThoughtIconMutation = useMutation({
        mutationKey: [thoughtApi.baseKey, "icon"],
        mutationFn: thoughtApi.updateThoughtIcon,
        onMutate: async (params) => {
            await queryClient.cancelQueries({ queryKey: [thoughtApi.baseKey] });
            ...
            queryClient.setQueryData(
                ...
                () => {
                    const newData = previousNotes.data?.map(task => (task as ThoughtResponce).thought_id === params.thoughtId ? { ...task, emoji: params.emoji } : task);
    ...
    return {
        updateThoughtIcon: updateThoughtIconMutation.mutate
    }
}