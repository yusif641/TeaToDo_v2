...
export const useUpdateThoughtText = (selectedId: string) => {
    const updateThoughtTextMutation = useMutation({
        mutationKey: [thoughtApi.baseKey, "text"],
        mutationFn: thoughtApi.updateThoughtText,
        onMutate: async (params) => {
            ...
            queryClient.setQueryData(
                ...
                () => {
                    const newData = previousNotes.data?.map(task => (task as ThoughtResponce).thought_id === params.thoughtId ? { ...task, text: params.text } : task);
    ...
    return {
        updateThoughtText: updateThoughtTextMutation.mutate
    }
}